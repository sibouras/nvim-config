...
local function cache_check()
  if vim.api.nvim_get_current_buf() == _cache_check.bufnr and vim.b.changedtick == _cache_check.changedtick then
    return
  end
  M.stack = {}
  _treeview_cache = nil
...
function M.up()
  ...
  if get_data(parent) == get_data(node) then
    return
  end
...
function M.next()
  ...
  if get_data(sibling) == get_data(node) then
    return
  end
  M.select(sibling)
end
